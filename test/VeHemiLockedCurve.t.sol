...
/// @title VeHemiLockedCurveTest
/// @notice Tests for VeHemi V2 locked curve: non-transferrable position weight tracking.
///
/// Test categories:
///   1. Initialization & Seeding (seedAndFinalizeLockedPositions)
///   2. Basic Locked Supply (nonTransferableTotalVeHemiSupply, supplyBreakdown)
///   3. increaseAmount (locked curve updated for non-transferrable positions)
...
///   6. Withdraw & Forfeit (locked curve decremented on position exit)
///   7. Historical Queries (nonTransferableTotalVeHemiSupplyAt)
///   8. Catchup Loop (locked curve tracks across SIX_DAYS boundaries)
///   9. Edge Cases & Reverts
contract VeHemiLockedCurveTest is LockedCurveTestBase {
    // ── Constants ────────────────────────────────────────────────────────
    ...
    function createLockedForfeitablePosition(
        address account_,
        uint256 amount_,
        uint256 duration_
    ) public returns (uint256 _tokenId, uint256 _slope, uint256 _end) {
    ...
    function test_SeedAndFinalize_WritesLockedSlopeChanges() public {
        ...
        // lockedSlopeChanges should have negative slope at each lock end
        assertEq(
            veHemi.lockedSlopeChanges(e1),
            -int128(int256(s1)),
            "lockedSlopeChanges at t1 end"
        );
        ...
        if (e1 == e2) {
            assertEq(
                veHemi.lockedSlopeChanges(e1),
                -int128(int256(s1 + s2)),
                "lockedSlopeChanges at shared end"
            );
        } else {
            assertEq(
                veHemi.lockedSlopeChanges(e2),
                -int128(int256(s2)),
                "lockedSlopeChanges at t2 end"
            );
        }
    }
    ...
    function test_NonTransferableTotalVeHemiSupply_ZeroBeforeFinalization() public view {
        // Before finalization, locked supply should be 0
        assertEq(
            veHemi.nonTransferableTotalVeHemiSupply(),
            0,
            "Locked supply should be 0 before finalization"
        );
    }
    ...
    function test_NonTransferableTotalVeHemiSupply_AfterFinalization() public {
        ...
        assertEq(lockedSupply, expectedBias, "Locked supply should match seeded bias");
    }
    ...
    function test_SupplyBreakdown_OnlyTransferable() public {
        ...
        // Seed and finalize with a dummy locked position first to enable tracking
        (uint256 t1,,) = createLockedPosition(bob, 10 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        assertGt(locked_, 0, "Locked should include bob's position");
    ...
    function test_SupplyBreakdown_LockedCappedAtTotal() public {
    ...
    function test_NonTransferableTotalVeHemiSupply_DecaysOverTime() public {
        ...
        assertLt(supplyLater, supplyNow, "Locked supply should decay over time");
    }

    function test_NonTransferableTotalVeHemiSupply_GoesToZeroAtExpiry() public {
        (uint256 t1,, uint256 e1) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        assertEq(
            veHemi.nonTransferableTotalVeHemiSupply(),
            0,
            "Locked supply should be 0 after all positions expire"
        );
    }
    ...
    function test_IncreaseAmount_UpdatesLockedCurve() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        assertGt(lockedAfter, lockedBefore, "Locked supply should increase after increaseAmount");
    }

    function test_IncreaseAmount_TransferableDoesNotAffectLockedCurve() public {
        (uint256 tLocked,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        (uint256 tTransferable,,) = createLock(bob, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(tLocked));
    ...
    function test_IncreaseAmount_LockedCurveMatchesExpected() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        assertEq(
            veHemi.nonTransferableTotalVeHemiSupply(),
            expectedBias,
            "Locked supply should match new slope * remaining time"
        );
    }
    ...
    function test_IncreaseUnlockTime_LockedSupplyUnchanged() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        // Locked supply should NOT change — subcurve is bounded by transferableAfter (original end)
        assertEq(lockedAfter, lockedBefore, "Locked supply should NOT change when extending lock");
    ...
    function test_IncreaseUnlockTime_TransferablePositionNoTransferableAfterEvent() public {
        ...
        (uint256 tLocked,,) = createLockedPosition(bob, 10 ether, LOCK_2Y);
        seedAndFinalize(_toArray(tLocked));
    ...
    function test_IncreaseUnlockTime_LockedSlopeChangeStaysAtOriginalEnd() public {
        (uint256 t1, uint256 s1,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        // Locked slope change should REMAIN at the original end (transferableAfter)
        int128 lockedSlopeAtOldEnd = veHemi.lockedSlopeChanges(oldEnd);
        assertLt(lockedSlopeAtOldEnd, 0, "lockedSlopeChanges at original end should still be negative");

        // NO locked slope change should exist at the new end
        int128 lockedSlopeAtNewEnd = veHemi.lockedSlopeChanges(newEnd);
        assertEq(lockedSlopeAtNewEnd, 0, "lockedSlopeChanges at new end should be 0");
    }
    ...
    function test_Withdraw_ReducesLockedCurve() public {
        (uint256 t1,, uint256 e1) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        (uint256 t2,,) = createLockedPosition(bob, 100 ether, MAX_TIME);
        seedAndFinalize(_toArray(t1, t2));
        ...
        // Withdraw — locked curve already decayed to 0 for this position,
        ...
        uint256 lockedAfter = veHemi.nonTransferableTotalVeHemiSupply();
        assertLt(lockedAfter, lockedBefore, "Locked supply should decrease after time + withdraw");
    }

    function test_Forfeit_ReducesLockedCurve() public {
        // Set forfeit admin
        veHemi.updateForfeitAdmin(admin);

        (uint256 t1,,) = createLockedForfeitablePosition(alice, 100 ether, LOCK_2Y);
        (uint256 t2,,) = createLockedPosition(bob, 100 ether, MAX_TIME);
        seedAndFinalize(_toArray(t1, t2));
        ...
        uint256 lockedAfter = veHemi.nonTransferableTotalVeHemiSupply();
        assertLt(lockedAfter, lockedBefore, "Locked supply should decrease after forfeit");
    }
    ...
    function test_NonTransferableTotalVeHemiSupplyAt_PastTimestamp() public {
        ...
        assertEq(pastSupply, supplyAtCreate, "Past locked supply should match snapshot");
    }
    ...
    function test_NonTransferableTotalVeHemiSupplyAt_FutureDecay() public {
        ...
        assertLt(supplyFuture, supplyNow, "Future locked supply should be less (decay)");

        // Query after expiry — should be 0
        uint256 supplyAfterExpiry = veHemi.nonTransferableTotalVeHemiSupplyAt(e1 + 1);
        assertEq(supplyAfterExpiry, 0, "Locked supply after expiry should be 0");
    }
    ...
    function test_NonTransferableTotalVeHemiSupplyAt_BeforeV2IsZero() public {
        ...
        assertEq(
            ...
            0,
            "Pre-V2 locked supply should be 0"
        );
    }
    ...
    function test_CatchupLoop_LockedCurveTracksAcrossBoundaries() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        // Locked supply should still be consistent with global tracking
        uint256 totalSupply = veHemi.totalVeHemiSupply();
        uint256 lockedSupply = veHemi.nonTransferableTotalVeHemiSupply();
        assertGt(lockedSupply, 0, "Locked supply should be positive after catchup");
        assertLe(lockedSupply, totalSupply, "Locked supply should not exceed total");
    }

    function test_CatchupLoop_LockedCurveConsistentWithGlobal() public {
        // Only locked positions — locked supply should equal total supply
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        for (uint256 i; i < 5; ++i) {
            ...
            assertEq(locked_, total, "Locked should equal total when only locked positions exist");
        }
    }
    ...
    function test_CatchupLoop_MixedPositionsConsistency() public {
        ...
        assertEq(forfeitable_, 0, "No forfeitable positions in this test");
            assertEq(total, locked_ + transferable, "total = locked + transferable");
            assertGt(locked_, 0, "Locked should be positive during lock");
            assertGt(transferable, 0, "Transferable should be positive during lock");
        }
    }

    function test_CatchupLoop_LargeTimeGap() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, MAX_TIME);
        seedAndFinalize(_toArray(t1));
        ...
        uint256 total = veHemi.totalVeHemiSupply();
        uint256 locked_ = veHemi.nonTransferableTotalVeHemiSupply();
        assertEq(locked_, total, "Locked should equal total with only locked positions");
        assertGt(locked_, 0, "Locked should still be positive (lock not expired)");
    }
    ...
    function test_NewLockedPosition_AfterFinalization_TrackedInLockedCurve() public {
        ...
        // Create a new locked position after finalization
        (uint256 t2,,) = createLockedPosition(bob, 200 ether, MAX_TIME);

        uint256 lockedAfter = veHemi.nonTransferableTotalVeHemiSupply();
        assertGt(lockedAfter, lockedBefore, "New locked position should be tracked");
    }

    function test_NewTransferablePosition_AfterFinalization_NotInLockedCurve() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
        ...
        uint256 lockedAfter = veHemi.nonTransferableTotalVeHemiSupply();
        assertEq(lockedAfter, lockedBefore, "Transferable position should not affect locked supply");
    }

    function test_LockedCurve_BeforeFinalization_NoTracking() public {
        // Create positions but don't finalize
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);

        // Locked supply should be 0 — tracking not active
        assertEq(veHemi.nonTransferableTotalVeHemiSupply(), 0, "No tracking before finalization");

        // Create another position
        createLockedPosition(bob, 200 ether, MAX_TIME);
        assertEq(veHemi.nonTransferableTotalVeHemiSupply(), 0, "Still no tracking");
    }

    function test_Checkpoint_DoesNotAffectLockedCurve_BeforeFinalization() public {
        ...
        assertEq(
            veHemi.nonTransferableTotalVeHemiSupply(),
            0,
            "Checkpoint before finalization should not create locked curve"
        );
    }
    ...
    function test_SeedAndFinalize_WorksAfterCheckpointAdvancesEpoch() public {
        ...
        // Locked supply should be correct at the current timestamp
        uint256 lockedSupply = veHemi.nonTransferableTotalVeHemiSupply();
        uint256 expectedBias = s1 * (e1 - block.timestamp) + s2 * (e2 - block.timestamp);
        assertEq(lockedSupply, expectedBias, "Locked supply should match after epoch advance");
    }

    function test_LockedSupplyBreakdown_AllPositionsExpired() public {
        (uint256 t1,, uint256 e1) = createLockedPosition(alice, 100 ether, 2 * SIX_DAYS);
        seedAndFinalize(_toArray(t1));
        ...
        assertEq(locked_, 0, "Locked should be 0");
    ...
    function testFuzz_LockedSupplyLeTotalSupply(
        ...
        (uint256 tLocked,,) = createLockedPosition(alice, lockedAmount, duration);
    ...
    function testFuzz_LockedCurveDecaysMonotonically(
        uint256 amount,
        uint256 duration
    ) public {
        ...
        (uint256 t1,,) = createLockedPosition(alice, amount, duration);
        seedAndFinalize(_toArray(t1));
        ...
        for (uint256 i; i < 5; ++i) {
            ...
            assertLe(currentSupply, prevSupply, "Locked supply must be monotonically non-increasing");
            prevSupply = currentSupply;
        }
    }

    function testFuzz_IncreaseAmount_LockedCurveIncrease(
        ...
        (uint256 t1,,) = createLockedPosition(alice, initialAmount, duration);
        seedAndFinalize(_toArray(t1));
        ...
        assertGt(supplyAfter, supplyBefore, "Locked supply must increase after increaseAmount");
    }

    // testFuzz_Slash_ReducesLockedCurve removed — requires restaking infrastructure

    // ═════════════════════════════════════════════════════════════════════
    //  11. ATOMICITY & IDEMPOTENCY
    // ═════════════════════════════════════════════════════════════════════

    function test_SeedAndFinalize_AllExpired_ZeroLockedSupply() public {
        // All positions expired before seeding — locked supply should be 0
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, 2 * SIX_DAYS);
        (uint256 t2,,) = createLockedPosition(bob, 200 ether, 2 * SIX_DAYS);
    ...
    function test_SeedAndFinalize_SameBlockAsLockCreation() public {
        ...
        uint256 lockedSupply = veHemi.nonTransferableTotalVeHemiSupply();
        uint256 expectedBias = s1 * (e1 - block.timestamp);
        assertEq(lockedSupply, expectedBias, "Same-block seed should work correctly");
    }
    ...
    function test_LockedEqualsGlobal_WhenOnlyLockedPositions() public {
    ...
    function test_TransferableEqualsGlobal_WhenNoLockedPositions() public {
    ...
    function test_LockedCurve_MultiplePositionsSameLockEnd() public {
        // Two locked positions with same lock end — slopes should stack
        (uint256 t1, uint256 s1,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        (uint256 t2, uint256 s2, uint256 e2) = createLockedPosition(bob, 200 ether, LOCK_2Y);
        ...
        int128 slopeChange = veHemi.lockedSlopeChanges(e1);
        assertEq(slopeChange, -int128(int256(s1 + s2)), "Slopes should stack at same end");
    }

    function test_LockedCurve_PositionExpiryMidCatchupLoop() public {
        // Position that expires during the catchup loop (between SIX_DAYS boundaries)
        (uint256 t1,, uint256 e1) = createLockedPosition(alice, 100 ether, 4 * SIX_DAYS);
        (uint256 t2,,) = createLockedPosition(bob, 200 ether, MAX_TIME);
        seedAndFinalize(_toArray(t1, t2));
        ...
        // Locked supply should only be bob's position now
        uint256 lockedSupply = veHemi.nonTransferableTotalVeHemiSupply();
        assertGt(lockedSupply, 0, "Bob's position should still have weight");
        ...
        assertEq(lockedSupply, expectedBias, "Should match bob-only expected bias");
    }
    ...
    function test_HistoricalLockedSupply_MonotonicallyDecreasing() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
    ...
    function test_HistoricalLockedSupply_AfterIncreaseAmount() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
    ...
    function test_SameBlockCheckpoints_LockedPointOverwritten() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));
    ...
    function test_DeprecatedStorageVarsNotExposed() public {
        ...
        // lockedSeedingFinalized should still be public
        (bool ok,) = address(veHemi).staticcall(
            abi.encodeWithSignature("lockedSeedingFinalized()")
        );
        assertTrue(ok, "lockedSeedingFinalized should still be public");
    }
    ...
    function test_IncreaseUnlockTime_MultipleTimes_LockedSupplyUnchanged() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));

        uint256 lockedAtCreation = veHemi.nonTransferableTotalVeHemiSupply();
        uint256 originalTransferableAfter = veHemi.transferableAfter(t1);
        ...
        assertEq(supply1, lockedAtCreation, "First extension should NOT change locked supply");
        ...
        assertEq(supply2, lockedAtCreation, "Second extension should NOT change locked supply");
        ...
        // Locked supply should be bounded by transferableAfter (original end), not lock.end
        uint256 expectedSlope = 100 ether / MAX_TIME;
        uint256 expectedBias = expectedSlope * (originalTransferableAfter - block.timestamp);
        assertEq(supply2, expectedBias, "Locked supply should be bounded by original transferableAfter");
        ...
        assertGt(veHemi.totalVeHemiSupply(), supply2, "Global supply should exceed locked supply");
    }

    function test_IncreaseUnlockTime_LockedSlopeChangeRemainsAtTransferableAfter() public {
        (uint256 t1, uint256 s1,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));

        uint256 oldEnd = veHemi.getLockedBalance(t1).end;
        int128 slopeChangeBeforeExtension = veHemi.lockedSlopeChanges(oldEnd);
        ...
        // Old end (transferableAfter) should STILL have the locked slope change — NOT cleared
        int128 slopeChangeAfterExtension = veHemi.lockedSlopeChanges(oldEnd);
        assertEq(
            slopeChangeAfterExtension,
            slopeChangeBeforeExtension,
            "Locked slope change at transferableAfter should be unchanged"
        );
        assertEq(
            slopeChangeAfterExtension,
            -int128(int256(s1)),
            "Locked slope change should equal position's negative slope"
        );

        // New end should have NO locked slope change
        assertEq(veHemi.lockedSlopeChanges(newEnd), 0, "New end should have no locked slope change");
    }
    ...
    function testFuzz_LockedEqualsTotalMinusTransferable(
        uint256 lockedAmt,
        ...
        (uint256 tLocked,,) = createLockedPosition(alice, lockedAmt, duration);
    ...
    function testFuzz_SeedAndFinalize_VariousAmountsAndDurations(
        ...
        (uint256 t1, uint256 s1, uint256 e1) = createLockedPosition(alice, amount1, dur1);
        (uint256 t2, uint256 s2, uint256 e2) = createLockedPosition(bob, amount2, dur2);

        seedAndFinalize(_toArray(t1, t2));

        uint256 lockedSupply = veHemi.nonTransferableTotalVeHemiSupply();
        uint256 expectedBias = s1 * (e1 - block.timestamp) + s2 * (e2 - block.timestamp);
        assertEq(lockedSupply, expectedBias, "Locked supply should match sum of biases");
    }
    ...
    function test_NewLockedPosition_AfterFinalization_UpdatesSlopeChanges() public {
        (uint256 t1,,) = createLockedPosition(alice, 100 ether, LOCK_2Y);
        seedAndFinalize(_toArray(t1));

        // Create a new non-transferable position AFTER seeding
        (uint256 t2,, uint256 e2) = createLockedPosition(bob, 200 ether, LOCK_3Y);

        // The new position's lock end should have a lockedSlopeChange entry
        int128 slopeChange = veHemi.lockedSlopeChanges(e2);
        assertLt(slopeChange, 0, "lockedSlopeChanges should have negative delta at new position's lock end");

        // After warping past SIX_DAYS boundaries and checkpointing,
        // the locked supply should decay correctly
        uint256 supplyBefore = veHemi.nonTransferableTotalVeHemiSupply();
        vm.warp(block.timestamp + 30 days);
        veHemi.checkpoint();
        uint256 supplyAfter = veHemi.nonTransferableTotalVeHemiSupply();
        assertLt(supplyAfter, supplyBefore, "Locked supply should decay after time warp with new position");
    }
    ...
    /// @notice Fuzz: increaseUnlockTime on locked positions does NOT change locked supply.
    function testFuzz_IncreaseUnlockTime_LockedCurveUnchanged(
        uint256 lockAmount,
        ...
        (uint256 t1,,) = createLockedPosition(alice, lockAmount, initialDuration);
        seedAndFinalize(_toArray(t1));
        ...
        // Locked supply should NOT change — bounded by original transferableAfter
        assertEq(lockedAfter, lockedBefore, "Locked supply must NOT change after extending lock");
